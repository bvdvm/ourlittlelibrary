...
export function initSagasPanel() {
  ...
  sagas.forEach(saga => {
    ...
    card.innerHTML = `
      ...
      <div class="saga-books">
        ${sagaBooks.length ? sagaBooks.map(b => `<span class="saga-book-pill">${escapeHtml(b.title)}</span>`).join('') : '<span class="saga-book-pill">jeszcze bez książek</span>'}
      </div>
    `;
